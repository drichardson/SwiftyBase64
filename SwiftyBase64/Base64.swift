...
public func EncodeString(bytes : [UInt8], alphabet : Alphabet = .Standard) -> String {
    let encoded = Encode(bytes: bytes, alphabet : alphabet)
    var result = String()
    for b in encoded {
...
public func Encode(bytes : [UInt8], alphabet : Alphabet = .Standard) -> [UInt8] {
    ...
    let table = tableForAlphabet(alphabet: alphabet)
    ...
    for c in stride(from: 0,to:count, by: 3) {
        let one = bytes[c] >> 2
        let two = ((bytes[c] & 0b11) << 4) | ((bytes[c+1] & 0b11110000) >> 4)
        let three = ((bytes[c+1] & 0b00001111) << 2) | ((bytes[c+2] & 0b11000000) >> 6)
        let four = bytes[c+2] & 0b00111111
